...
if (!subscriptions?.length) {
  console.log("No starter subscriptions found.");
  process.exit(0);
}

for (const sub of subscriptions) {
  console.log(`Resetting org ${sub.organization_id} (${sub.stripe_subscription_id ?? "no stripe sub"})`);
...
console.log(`Done. Reset ${subscriptions.length} starter subscription(s).`);
