    ...
    <form action={formAction} className="space-y-6">
      ...
      <Tabs defaultValue="basics">
        ...
        <TabsPanel value="specs">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <L label="Fuel *"><select name="fuelType" required defaultValue={v.fuel_type} className={inputCls}>{fuelTypes.map((f) => <option key={f} value={f}>{FUEL_LABELS[f]}</option>)}</select></L>
            <L label="Transmission *"><select name="transmission" required defaultValue={v.transmission} className={inputCls}>{transmissionTypes.map((t) => <option key={t} value={t}>{TRANSMISSION_LABELS[t]}</option>)}</select></L>
            <L label="Body type *"><select name="bodyType" required defaultValue={v.body_type} className={inputCls}>{bodyTypes.map((b) => <option key={b} value={b}>{BODY_TYPE_LABELS[b]}</option>)}</select></L>
